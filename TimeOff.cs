...
namespace Chatbot_Application
{
    public partial class TimeOff : Form
    {
        ...
        public TimeOff(string text)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT DISTINCT ShiftID, Shift_Date, Shift_Start, Shift_End FROM Shift WHERE EmployeeID = " + empID + " AND Worked = " + 1, conn);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    string query = "SELECT Shift_Date from Shift WHERE Shift_Date = '" + textBox1.Text + "' AND EmployeeID = " + empID;
                    ...
                    if (readerReturnValue.HasRows == true)
                    {
                        ...
                        DialogResult result = MessageBox.Show("Would you like take " + textBox1.Text + " off?", "Confirmation", buttons);
                        ...
                        else
                        {
                            if(textBox2.Text != null)
                            {
                                ...
                                conn.Open();
                                textBox1.Text = monthCalendar1.SelectionRange.Start.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);
                                SqlCommand cmd2 = new SqlCommand("SELECT ShiftID from Shift WHERE Shift_Date = '" + textBox1.Text + "' AND EmployeeID = " + empID);
                                ...
                                SqlCommand cmd3 = new SqlCommand("insert into Request values('"+ monthCalendar1.TodayDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture) + "', '" + 0 + "', '" + textBox2.Text  + "', '" + x + "')", conn);
                                cmd3.ExecuteNonQuery();
                                SqlCommand cmd4 = new SqlCommand("update Shift set Worked = " + 0 + "where ShiftID =" + x, conn);
